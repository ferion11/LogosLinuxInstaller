...
case "${productChoice}" in
    ...
    2*)
        ...
        FLPRODUCT="Verbum"
        FLPRODUCTi="verbum"
	if [ -z "${LOGOS64_URL}" ]; then export LOGOS64_URL="https://downloads.logoscdn.com/LBS9/Verbum/Installer/9.17.0.0010/${FLPRODUCT}-x64.msi" ; fi
        ;;
    3*)
        exit
        ;;
...
if [ -z "${INSTALLDIR}" ]; then export INSTALLDIR="${HOME}/${FLPRODUCT}Bible9" ; fi
...
case "${installationChoice}" in
	...
	2*)
		...
		;;
	3*)
		echo "Installing ${FLPRODUCT}Bible 64bits using ${WINE64_APPIMAGE_VERSION} plain AppImage without dependencies..."
		export WINEARCH=win64
		export WINEPREFIX="${APPDIR}/wine64_bottle"
		export WINE_EXE="wine64"
...
echo "================================================="
# Geting and install the ${FLPRODUCT}Bible:
